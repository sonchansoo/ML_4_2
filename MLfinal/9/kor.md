...
◼ 따라서 동전 A에서 나올 것으로 예상되는 앞면 수 = 50.45, 뒷면 수 = 50.45 (관찰 1에 5 H와 5 T가 있으므로)
◼ 유사하게, 동전 B에서 나올 것으로 예상되는 앞면 수 = 50.55, 뒷면 수 = 50.55.
...
대각 행렬 Σ를 구성함 (특이값을 대각선을 따라 내림차순으로 배치).
Σ-1을 계산함.
...
왼쪽 직교 행렬 U를 U = AVΣ-1로 구성함.

A = UΣVT를 사용하여 SVD를 계산함.
...
◼ 0 0 10.176=0.176 0 10.176=0.176 1*0.176=0.176
